# -*- coding: utf-8 -*-
"""
Created on Fri Mar 15 12:39:47 2019
...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 15 12:37:58 2019
...
print('test_size: %.2f' % pm.test_size)
print('num_filters:%i' % pm.num_filters)
print('filter_sizes: ')
print(pm.filter_sizes)
print('learning rate:%.5f' % pm.lr)
print('l2_constraint: %s' % pm.l2_constraint)
print('dropout_percent: %.2f' % pm.dropout_percent)
print('num_epoch: %i' % pm.num_epoch)
print('batch_size %i' % pm.batch_size)
...
def create_model(embedding_matrix, num_words, max_tokens, filter_sizes,
                 lr, num_filters, embedding_size, dropout_percent, l2_constraint):
    """
    ...
    dropout_percent: float, optional, default 0.6
...
def main():
    ...
    measurements =['test_size','num_filters','filter_sizes','lr','l2_constraint','dropout_percent','num_epoch','batch_size','Accuracy', 'Precision', 'Recall']
    result = pd.DataFrame([[pm.test_size,pm.num_filters,pm.filter_sizes,pm.lr,pm.l2_constraint,pm.dropout_percent,pm.num_epoch,pm.batch_size, accuracy, precision, recall]], columns = measurements)
    
    result.to_csv('results.csv', index=True, mode=  'a', header=True)
